...
def get_config() -> mlc.FrozenConfigDict:
    ...
    config.model_name = "CoAtNet0"
    config.model = models.coatnet.model(config)
    config.optimizer = models.coatnet.optimizer(config)
    config.transform = models.coatnet.preprocessing(config)
